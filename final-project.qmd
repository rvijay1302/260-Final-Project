...
weekly_counts_combined <- weekly_counts_combined %>% 
  mutate(
    predicted_rate = predictions1$fit,
    se = predictions1$se.fit,
    lower = predicted_rate - 1.96 * se,
    upper = predicted_rate + 1.96 * se
  )

ggplot(weekly_counts_combined, aes(x = date)) +
  geom_line(aes(y = predicted_rate, color = age_category), size = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = age_category), alpha = 0.2) +
  facet_grid(age_category ~ sex, scales = "free_y") +
  labs(
    title = "Predicted Rates with 95% Confidence Intervals",
    x = "Date",
    y = "Predicted Rate"
  ) +
  theme_minimal() +
...
weekly_counts_combined %>%
  ggplot(aes(x = date)) +
  geom_line(aes(y = rate, color = sex), alpha = 0.5) +
  geom_line(aes(y = predicted_rate, color = sex), linetype = "dashed") +
  facet_wrap(~ age_category, scales = "free_y", ncol = 1) +
  labs(
    title = "Observed vs Predicted Mortality Rates by Age Category and Sex",
    x = "Date",
    y = "Rate",
    color = "Sex"
  ) +
